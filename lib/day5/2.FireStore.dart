...
class _MyAppState extends State<MyApp> {
  ...
  void _deleteUser() async {
    ...
    QuerySnapshot snap = await users.where('name', isEqualTo: _name.text).get();
  ...
  void _updateUser() async {
    ...
    for (QueryDocumentSnapshot doc in snap.docs) {
      users.doc(doc.id).update({'age': 31}); // 나이 값을 30으로 업데이트
    }
  }
  ...
  Widget _listUser() {
    return StreamBuilder(
        stream: FirebaseFirestore.instance.collection("users").snapshots(), // 사용자 목록 스트림(snapshots은 현재시점에서 보여주는 명령어)
